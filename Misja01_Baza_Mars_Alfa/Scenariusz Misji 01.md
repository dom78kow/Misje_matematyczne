# Misja 01 -- Baza Mars Alfa

# Rozdział 1 -- Pierwszy krok na Marsie

Rok **2048**.

Po siedmiu miesiącach podróży statek **Explorer I** zbliża się do
powierzchni Marsa. Za kilka minut rozpocznie się lądowanie pierwszej
szkolnej ekspedycji badawczej.

Przez iluminator widzisz czerwoną równinę i ogromny **Wulkan Argo**.

Komputer odlicza:

> 120 sekund do lądowania...

Nagle światła gasną.

Alarm.

> **AWARIA KOMPUTERA POKŁADOWEGO**

Po kilku sekundach statek ląduje bezpiecznie, jednak komputer
odpowiedzialny za planowanie misji przestaje działać.

Profesor Jan Kowalski zwraca się do Ciebie:

> „Od tej chwili wszystkie decyzje będziemy podejmować sami."

Na ekranie pojawia się wiadomość z Centrum Dowodzenia:

> **Powodzenie misji zależy od poprawnych obliczeń i właściwej analizy
> dokumentów.**

Przed Tobą pierwsza teczka.

**M01-D01 -- Manifest transportowy**

------------------------------------------------------------------------

# Rozdział 2 -- Pierwszy transport

### Komunikat

Nie można rozpocząć rozładunku, dopóki nie zostanie sprawdzona masa
całego transportu.

**Przeczytaj dokument M01-D01 -- Manifest transportowy.**

## Zadania

1.  Wskaż informacje potrzebne do obliczeń.
2.  Wskaż informacje zbędne.
3.  Oblicz całkowitą masę ładunku.
4.  Sprawdź, czy jeden lot wystarczy.
5.  Jeżeli nie -- zaproponuj podział transportu.
6.  Uzasadnij swoją decyzję.

------------------------------------------------------------------------

# Rozdział 3 -- Pierwsza noc na Marsie

Po rozładunku okazuje się, że do zachodu Słońca pozostało niewiele
czasu.

Bez energii baza nie przetrwa nocy.

**Przeczytaj dokument M01-D02 -- Raport energetyczny.**

## Zadania

1.  Oblicz produkcję energii.
2.  Oblicz zużycie energii.
3.  Porównaj oba wyniki.
4.  Zdecyduj, które urządzenia uruchomisz.
5.  Uzasadnij wybór.

------------------------------------------------------------------------

# Rozdział 4 -- Dom dla sprzętu

Rozładowany sprzęt nadal znajduje się na zewnątrz.

Zbliża się burza pyłowa.

Konieczne jest przygotowanie magazynu.

**Przeczytaj dokument M01-D03 -- Plan bazy.**

## Zadania

1.  Oblicz pole podłogi magazynu.
2.  Oblicz jego objętość.
3.  Oceń, czy pomieści cały sprzęt.
4.  Zaproponuj jedno usprawnienie projektu.

------------------------------------------------------------------------

# Rozdział 5 -- Ile przetrwamy?

Centrum Dowodzenia informuje o możliwym opóźnieniu następnego
transportu.

Załoga musi wiedzieć, jak długo może funkcjonować samodzielnie.

**Przeczytaj dokument M01-D04 -- Raport zapasów.**

## Zadania

1.  Na ile dni wystarczy woda?
2.  Na ile dni wystarczy żywność?
3.  Którego zasobu zabraknie wcześniej?
4.  Zaproponuj dwa sposoby ograniczenia zużycia.

------------------------------------------------------------------------

# Rozdział 6 -- Ostatnie przygotowania

Baza jest prawie gotowa.

Pozostało zatwierdzić rozmieszczenie obiektów.

**Korzystaj z dokumentów M01-D03 i M01-D05.**

## Zadania

1.  Odczytaj odległości ze skali.
2.  Wybierz najlepsze miejsce dla magazynu.
3.  Uzasadnij rozmieszczenie paneli słonecznych.
4.  Wyznacz najkrótszą trasę transportu.

------------------------------------------------------------------------

# Rozdział 7 -- Decyzja Dowódcy

Profesor Kowalski przygotowuje raport dla Centrum Dowodzenia.

Potrzebuje Twojej opinii.

## Zadania

1.  Czy baza jest gotowa do rozpoczęcia pracy?
2.  Podaj co najmniej pięć argumentów.
3.  Wskaż trzy elementy wymagające poprawy.
4.  Przygotuj krótki raport końcowy.

------------------------------------------------------------------------

# Rozdział 8 -- Tajemniczy sygnał

Podczas pierwszego odwiertu czujniki rejestrują powtarzalny sygnał.

Technicy nie potrafią wyjaśnić jego pochodzenia.

**Przeczytaj dokument M01-D06 -- Raport z odwiertu.**

## Zadania

1.  Wypisz najważniejsze informacje z raportu.
2.  Postaw hipotezę dotyczącą źródła sygnału.
3.  Jak przygotowałbyś wyprawę do Wulkanu Argo?

Na ekranie pojawia się komunikat:

> **ŹRÓDŁO SYGNAŁU ZLOKALIZOWANE**

> **ODLEGŁOŚĆ: 12 km**

> **KIERUNEK: WULKAN ARGO**

**Koniec Misji 01.**

**Ciąg dalszy: Misja 02 -- Wyprawa do wnętrza wulkanu.**
